...
export default function ApprovalStatusPage() {
  ...
  let description = "Processing your approval...";
  let IconComponent = AlertTriangle;
  let cardVariant: "default" | "success" | "destructive" | "warning" = "default";

  if (status === "success") {
    title = "Approval Successful";
    description = "The user account has been successfully approved and activated. They can now log in if their email is verified.";
    IconComponent = CheckCircle;
    cardVariant = "success";
  } else if (status === "invalid") {
    title = "Invalid or Expired Link";
    description = `This approval link is invalid, has expired, or the user has already been processed. ${message ? `Details: ${message.replace(/_/g, ' ')}` : 'Please contact support if you believe this is an error.'}`;
    IconComponent = XCircle;
    cardVariant = "destructive";
  } else if (status === "error") {
    title = "Approval Error";
    description = "An unexpected error occurred while processing the approval. Please try again later or contact support.";
