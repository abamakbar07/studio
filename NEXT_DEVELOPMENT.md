...
*   **Project-Scoped Firebase Security Rules:**
    *   Conduct a thorough review and strengthening of Firebase Security Rules for Firestore to ensure least-privilege access. **[Initial rules conceptualized for `sto_projects` collection and basic user data; needs full implementation for project-scoped data access by assigned admins.]**
...
*   **B. SOH Data Processing & Storage (Per Project):**
    *   SOH data uploads are now **scoped to a specific STO Project**. **[DONE - UI for superusers to select project for SOH upload implemented. Admin uploads are scoped to their selected project. Actual backend processing/storage of XLSX is still simulated.]**
    *   A project can have multiple SOH data references (e.g., initial SOH, supplementary SOH). **[Conceptually supported; UI for managing multiple references per project TBD.]**
...
*   **F. Dashboard Enhancements for Project Monitoring:**
    ...
    *   Implement a dynamic "Recent Activity" feed on the dashboard, filterable by project, showing key actions like project creations/status changes, form creations, SOH uploads, and user approvals related to the selected project. **[Placeholder implemented, actual feed pending.]**
...
*   **G. User Management Enhancements (Project Assignment Focus):**
    ...
    *   Admin Users' views and actions within the application (e.g., accessing forms, inputting data) are **restricted to the STO Projects they are assigned to**. **[DONE - Implemented project selection workflow for admins. Relevant pages show project context. Data filtering per project is the next step for each module.]**
...
*   **UI/UX Refinements:**
    *   **Project Context:** Ensure the current STO Project context is always clearly visible and navigable when users are working on project-specific data (forms, SOH, reports). **[DONE - Admins select a project, context is shown in header and relevant pages. Superuser project selection for specific views like SOH upload also implemented.]**
    ...
    *   Improve loading states, progress indicators, and user feedback across the application. **[Ongoing, initial states implemented.]**
...
*   **Data Structures for Project-Based Architecture:**
    *   Firestore data structures (collections, documents) must be designed to efficiently support project-based querying and data isolation (e.g., using `projectId` as a key field in relevant documents). **[Initial structure for `sto_projects` collection implemented. User assignment via `assignedAdminUserIds` on project doc done.]**
...
*   **End-to-End (E2E) Tests:**
    *   Implement E2E tests for key user flows, ensuring they correctly handle the project-based architecture (project selection, project-scoped data operations, user assignments to projects). **[Manual E2E testing for admin project selection workflow conducted.]**
